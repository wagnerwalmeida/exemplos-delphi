...
begin
  ...
  with DataModulo.IBTable do
    if Locate('Codigo', edtCodigo.Text, [loCaseInsensitive]) then
    begin
...
begin
  ...
  begin
    // localiza o produto para dar baixa
    DataModulo.IBTable.Locate('Codigo', TabelaTemporaria.FieldByName('Codigo').AsInteger, []);

    // coloca a tabela em modo de alteração
    DataModulo.IBTable.Edit;

    // altera o estoque (estoque atual menos a quantidade contida na tabela temporária)
    DataModulo.IBTable.FieldByName('Estoque').AsInteger :=
      DataModulo.IBTable.FieldByName('Estoque').AsInteger - TabelaTemporaria.FieldByName('Quantidade').AsInteger;

    // grava a alteração
    DataModulo.IBTable.Post;

    // posicionar no próximo registro da tabela temporária
